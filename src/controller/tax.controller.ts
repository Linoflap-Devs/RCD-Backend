...
export const getAgentTaxRateController = async (req: Request, res: Response) => {
    ...
    const result = await getAgentTaxRatesService({ agentTaxRateIds: [Number(agentTaxRateId)], showInactive: true, showDetails: true })

    if(!result.success) {
        res.status(result.error?.code || 400).json({success: false, data: {}, message: result.error?.message || 'Failed to get agent tax rate'})
        return;
    }

    res.status(200).json({success: true, message: 'Agent tax rate details.', data: result.data })
}
