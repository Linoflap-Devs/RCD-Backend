...
export const getWebPendingSalesController = async (req: Request, res: Response) => {
    ...
    const { page, pageSize, month, year, developerId, search, status } = req.query
...
export const archivePendingSalesController = async (req: Request, res: Response) => {
    ...
    const result = await archivePendingSaleService(session.userID, Number(pendingSalesId))

    if(!result.success){
        res.status(result.error?.code || 500).json({success: false, message: result.error?.message || 'Failed to archive sales', data: {}})
        return;
    }

    return res.status(200).json({success: true, message: 'Sales archived', data: result.data})
}
...
export const archiveWebPendingSalesController = async (req: Request, res: Response) => {
    const session = req.session
...
export const editSaleImagesController = async (req: Request, res: Response) => {
    ...
    const result = await editPendingSaleImagesServiceR2(
...
export const getDivisionSalesTotalsYearlyFnController = async (req: Request, res: Response) => {
...
export const getSalesByDeveloperTotalsFnController = async (req: Request, res: Response) => {
    ...
    const result = await getSalesByDeveloperTotalsFnService(session.userID, {
        month: month ? Number(month) : undefined,
        year: year ? Number(year) : undefined
    });
...
export const addDistributionListController = async (req: Request, res: Response) => {
    const session = req.session
    ...
    const result = await addSalesDistributionListService(session.userID, {
