...
export const revokeInviteTokenController = async (req: Request, res: Response) => {
    const session = req.session 
    ...
    const result = await revokeInviteTokenService(session.userID, inviteToken);

    if(!result.success) {
        res.status(result.error?.code || 500).json({
            success: false, 
            message: result.error?.message || "Failed to revoke invite token.", 
            data: {}
        });
    }
}
