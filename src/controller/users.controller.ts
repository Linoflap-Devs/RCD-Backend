...
export const editAgentGovIdsController = async (req: Request, res: Response) => {
    const session = req.session
    ...
    const result = await editAgentGovIdsService(Number(session.userID), req.body)

    if(!result.success){
        res.status(400).json({ 
            success: false,
            message: result.error?.message || "Failed to edit user gov ids.",
            data: {}
         });
        return
    }
...
export const editBrokerGovIdsController = async (req: Request, res: Response) => {
    const session = req.session
    ...
    const result = await editBrokerGovIdsService(Number(session.userID), req.body)
